...
final class ProbabilityEngine {
    static final class Input {
        ...
        // Independent model inputs. 1.0 = league-average strength / neutral lineup.
        double homeStrength=1.0, awayStrength=1.0;
        double homeOpponentStrength=1.0, awayOpponentStrength=1.0;
        double homeLineupAttack=1.0, homeLineupDefence=1.0;
        double awayLineupAttack=1.0, awayLineupDefence=1.0;
        boolean homeLineupConfirmed, awayLineupConfirmed;
    ...
    static Result calculate(Input x) {
        ...
        // Team quality is independent of bookmaker odds. Opponent-strength adjustment prevents
        // raw form against weak opposition from being treated the same as form against strong opposition.
        double hs=clamp(x.homeStrength,.72,1.35), as=clamp(x.awayStrength,.72,1.35);
        double ho=clamp(x.homeOpponentStrength,.78,1.25), ao=clamp(x.awayOpponentStrength,.78,1.25);
        lh*=clamp(Math.pow(hs/as,.42)*Math.pow(ho/ao,.16),.72,1.38);
        la*=clamp(Math.pow(as/hs,.42)*Math.pow(ao/ho,.16),.72,1.38);

        // Confirmed lineups affect attack and the opponent's defensive resistance.
        // Unconfirmed projected lineups are deliberately shrunk towards neutral.
        double hla=clamp(x.homeLineupAttack,.75,1.25), hld=clamp(x.homeLineupDefence,.75,1.25);
        double ala=clamp(x.awayLineupAttack,.75,1.25), ald=clamp(x.awayLineupDefence,.75,1.25);
        double lineupWeight=(x.homeLineupConfirmed&&x.awayLineupConfirmed)?1.0:.35;
        lh*=clamp(1+lineupWeight*((hla/ald)-1),.78,1.25);
        la*=clamp(1+lineupWeight*((ala/hld)-1),.78,1.25);

        lh=clamp(lh,0.20,3.60);la=clamp(la,0.15,3.30);
        r.lambdaH=lh;r.lambdaA=la;
        int quality=30; quality+=Math.min(20,Math.min(x.homeN,x.awayN)*2);
        if(x.homeVenueN>=3&&x.awayVenueN>=3)quality+=12;
        if(x.homeXgN>=3&&x.awayXgN>=3)quality+=18;
        if(x.homeSotN>=3&&x.awaySotN>=3)quality+=8;
        if(Math.abs(x.homeStrength-1)>0.01||Math.abs(x.awayStrength-1)>0.01)quality+=5;
        if(x.homeLineupConfirmed&&x.awayLineupConfirmed)quality+=7;
        r.reliability=Math.min(100,quality);
